...
class LikeGenreAdapter(
    private var context: Context,
    private var nowShowingList: ArrayList<PreferenceResponse.Output.Genre.Other>,
    private var listener: RecycleViewItemClickListener
) : RecyclerView.Adapter<LikeGenreAdapter.ViewHolder>() {
    private var rowIndex = 0

    inner class ViewHolder(val binding: GenreChildItemBinding) :
        RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding =
            GenreChildItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
...
//                    nowShowingList.removeAt(position)
    ...
    interface RecycleViewItemClickListener {
        fun genreLikeClick(comingSoonItem: PreferenceResponse.Output.Genre.Other)
    }
