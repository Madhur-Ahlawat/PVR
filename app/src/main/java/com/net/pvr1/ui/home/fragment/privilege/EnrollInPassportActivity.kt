...
import com.net.pvr1.databinding.ActivityEnrollPassportBinding
...
@AndroidEntryPoint
class EnrollInPassportActivity : AppCompatActivity() {
    @Inject
    ...
    private fun passportSave() {
        authViewModel.passportSaveLiveData.observe(this) {
            when (it) {
                is NetworkResult.Success -> {
                    if (Constant.status == it.data?.result && Constant.SUCCESS_CODE == it.data.code) {
                        Constant.BOOKING_ID = it.data.output.id
                        Constant.BOOK_TYPE = it.data.output.booktype
                        val intent = Intent(this@EnrollInPassportActivity, PaymentActivity::class.java)
                        intent.putExtra("paidAmount",(it.data.output.ta.toDouble()/100.0).toString())
